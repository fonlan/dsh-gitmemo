...
function registerMemTools(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        name: "mem_read",
        description: "Read one gitmemo memory entry by commit hash (returned by mem_search). Select only the most relevant memories before reading — at most 5 when a search returns more. Historical entries that were replaced or deleted stay readable for audit.",
        parameters: {
            commit_hash: {
                type: "string",
                required: true,
        ...
        output: {
            ...
            render: (_args, value) => textBlock((value.legacy === true ? "[legacy entry]\n" : "") + value.content)
        },
        isConcurrencySafe: () => true,
        async execute(args, exec) {
            const memo = await engineFor(exec, config);
    ...
    ctx.tools.register(defineTool({
        name: "mem_write",
        description: "Store a completed task outcome in the gitmemo long-term memory (.mem git repo). Write ONLY when the task is complete AND repo-related AND the outcome is valuable/reusable OR the user explicitly asked to remember. Entries are immutable: title (single line, ≤200 chars, \"[module] action + object\"), summary (≤4000 chars), keywords (2-12, include 中英文同义词) and content (markdown body; the engine generates front matter) are all required; related_branches/related_paths are optional. Keywords must ADD new search angles — pick task-related terms (中英文同义词 both fine) that are NOT already present in title/summary: words already in title/summary are searchable via those fields themselves, so repeating them does not improve recall. Corrections use mem_replace, withdrawal uses mem_delete — never rewrite in place.",
        parameters: {
            title: {
                type: "string",
                required: true,
    ...
    ctx.tools.register(defineTool({
        name: "mem_replace",
        description: "Replace an active gitmemo memory entry with a new conclusion in ONE atomic commit (deletes the old entry file, adds the new one). Use when the user corrects a stored outcome — never delete-then-write. commit_hash must be the ACTIVE hash returned by mem_search; stale hashes are rejected. Takes the same fields as mem_write.",
        parameters: {
            commit_hash: {
                type: "string",
                required: true,
