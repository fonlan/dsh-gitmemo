...
function registerPromptSection(ctx) {
    ctx.systemPrompt.section({
        ...
        text: [
            ...
            "- END-OF-SESSION CHECKPOINT — the ONLY write path: when the user says \"no more tasks\", \"that's all\", or the conversation is ending, review the whole session and mem_write EVERY completed repo-related task that still lacks a memory and whose outcome is valuable and reusable OR was explicitly asked to be remembered. Never write for pure Q&A, incomplete tasks, non-repo work, or purely operational git actions (commit/push only). Never duplicate an already-written entry; if a stored outcome is outdated, mem_delete it first, then write the corrected entry. Entry title: \"[module] action + object\"; content: markdown with YAML front matter (date, status, repo_branch, mem_branch, related_paths, tags) and Original User Request / AI Understanding / Final Outcome sections."
        ].join("\n")
    });
}
/**
