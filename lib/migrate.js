...
import { ENTRY_DIR, FORMAT_FILE, GITMEMO_FORMAT_VERSION, GitMemoError, LOCK_FILE_NAME, MIGRATION_JOURNAL_NAME, buildAddMessage, buildEntryMarkdown, computeDigest, createEntryFile, git, gitInitMain, normalizeText, repoFormat, resolveCommit, recoverMigrationSwap, slugifyTitle, commitWithIdentity, utcMs, withLock, writeMigrationSwapJournal } from "./mem.js";
function safeLegacyDate(epochRaw) {
    const ms = Number(epochRaw ?? 0) * 1000;
    return Number.isFinite(ms) && Math.abs(ms) <= 8.64e15 ? new Date(ms) : new Date(0);
}
async function isGitRepo(dir, gitTimeoutMs) {
    try {
...
async function listTips(memDir, gitTimeoutMs) {
...
function legacyFrontMatterKeywords(content) {
...
function legacyBodyKeywords(body) {
    const out = [];
    for (const line of body.split("\n")) {
        ...
        if (match === null)
            continue;
        if (!/[,，]/.test(match[2]))
            continue; // only accept list-like values
        out.push(...match[2]
            .split(/[,，]/)
            .map((s) => s.trim())
            .filter((s) => s.length > 0));
...
function finalOutcomeParagraph(content) {
    const lines = content.split("\n");
    let start = -1;
...
function inferLegacyMetadata(content, subject, body) {
    const warnings = [];
    ...
    if (keywords.length === 0)
        keywords = legacyBodyKeywords(body);
    const safeKeywords = [];
    ...
    for (const original of keywords) {
        ...
        if (bounded !== original.trim())
            warnings.push(`sanitized legacy keyword: ${JSON.stringify(original)}`);
        const normalized = normalizeText(bounded);
        if (seenKeywords.has(normalized))
            continue;
        seenKeywords.add(normalized);
        safeKeywords.push(bounded);
        if (safeKeywords.length === 12)
            break;
    }
    keywords = safeKeywords;
    if (keywords.length === 0)
        warnings.push("no keywords could be inferred deterministically (legacy entries allow 0-12)");
    let summary = finalOutcomeParagraph(content);
    if (summary.length === 0)
        summary = body.replace(/\n{3,}/g, "\n\n").trim();
    summary = summary.slice(0, 4000);
    if (summary.length === 0)
        warnings.push("no summary could be inferred");
 ...
 */
export async function migrateDryRun(root, options = {}) {
...
async function migrateDryRunUnlocked(root, options = {}) {
    ...
    const occurrences = new Map(); // content digest -> paths
...
// apply
// ---------------------------------------------------------------------------
function conflictManifest(conflicts) {
    return conflicts
        .map((c) => {
 ...
 */
export async function migrateApply(root, baseline, options = {}) {
    const lockTimeoutMs = options.lockTimeoutMs ?? 30000;
...
async function migrateApplyUnlocked(root, baseline, options = {}) {
    const gitTimeoutMs = options.gitTimeoutMs ?? 60000;
    if (baseline === null ||
    ...
    if (resolve(baseline.root) !== resolve(root)) {
        throw new GitMemoError(`gitmemo: migration baseline belongs to a different project root (${baseline.root}); re-run --dry-run for ${resolve(root)}`);
    }
    ...
    for (const branch of baselineBranches) {
        if (current.tips[branch] !== baseline.tips[branch]) {
            throw new GitMemoError(`gitmemo: branch "${branch}" changed since the dry-run baseline (${current.tips[branch] ?? "gone"} ≠ ${baseline.tips[branch]}) — re-run --dry-run`);
        }
    }
    if (current.branches.length !== baselineBranches.length) {
        throw new GitMemoError("gitmemo: branch set changed since the dry-run baseline — re-run --dry-run");
    }
    // 2. conflicts must be resolved by the user first
    if (baseline.conflicts.length > 0) {
        throw new GitMemoError(`gitmemo: migration blocked by ${baseline.conflicts.length} conflict(s) (same entry path, different content across branches):\n` +
    ...
    if (legacyStatus.length > 0) {
        throw new GitMemoError("gitmemo: legacy memory repository is not clean — migration refuses to discard uncommitted/staged content. " +
    ...
    try {
        ...
        throw new GitMemoError(`gitmemo: legacy repository contains an interrupted transaction journal (${legacyJournal}); repair it before migration`);
    }
    catch (error) {
    ...
    try {
        ...
        await git(tmp, ["fetch", "-q", memDir, `+refs/heads/*:refs/gitmemo/backup/${ts}/*`], gitTimeoutMs);
        // 5. one immutable ADD commit per unique entry, preserving old dates
        const entries = [...baseline.entries].sort((a, b) => a.date.localeCompare(b.date) || a.sourcePath.localeCompare(b.sourcePath));
        ...
        for (const entry of entries) {
            ...
            }, { branch: entry.codeBranch, commit: entry.codeCommit, relatedBranches: entry.relatedBranches }, entry.digest, entryDate);
            const file = await createEntryFile(tmp, baseName, markdown);
            await git(tmp, ["add", "--", file], gitTimeoutMs);
            const hash = await commitWithIdentity(tmp, buildAddMessage(entry.title, entry.summary, entry.keywords, entry.digest, true), gitTimeoutMs, { env });
        ...
        try {
            ...
            try {
                await recoverMigrationSwap(root, gitTimeoutMs);
                if ((await repoFormat(targetDir, gitTimeoutMs)) !== "new")
                    throw error;
            }
            catch (restoreError) {
                throw new GitMemoError(`gitmemo: migration swap failed and automatic restore also failed. Old repository remains at ${oldDir}; ` +
 ...
 */
export async function migrateAutoUnlocked(root, options = {}) {
    const gitTimeoutMs = options.gitTimeoutMs ?? 60000;
    const memDir = resolve(options.source ?? join(root, ".mem"));
    await recoverMigrationSwap(root, gitTimeoutMs);
    ...
    try {
        ...
        if (error instanceof GitMemoError) {
            const code = /no branches/.test(error.message) ? "no-branches" : "other";
    ...
    if (report.conflictCount > 0) {
        return {
            ...
            blocked: {
                code: "conflicts",
                detail: `${report.conflictCount} conflict(s) (same entry path, different content across branches):\n` +
    ...
    try {
        ...
        if (error instanceof GitMemoError) {
            ...
            let code = "other";
            if (/blocked by \d+ conflict/.test(message))
                code = "conflicts";
            else if (/not clean.*refuses to discard/.test(message))
                code = "dirty";
            else if (/interrupted transaction journal/.test(message))
                code = "legacy-journal";
            else if (/baseline contains no branches/.test(message))
                code = "no-branches";
