 ...
 */
export function computeDigest(title, summary, keywords, content) {
...
export function buildAddMessage(title, summary, keywords, digest, legacy = false) {
...
export function buildReplaceMessage(title, summary, keywords, digest, replacesHash) {
...
function legacyWarning(root) {
    return ("gitmemo: .mem is in legacy format — search is read-only over the current branch. " +
...
async function assertNewFormat(memDir, gitTimeoutMs, root) {
    const format = await repoFormat(memDir, gitTimeoutMs);
    if (format !== "new") {
        throw new GitMemoError("gitmemo: .mem exists in legacy format — write operations are disabled. " +
            `Run \`dsh-gitmemo migrate --project-root ${root} --dry-run\` to plan migration, then --apply.`);
    }
}
    ...
    async write(input, legacy = false) {
        ...
        return withLock(this.root, this.config, "write", async () => {
            ...
            try {
                ...
                const hash = await commitWithIdentity(this.memDir, buildAddMessage(validated.title, validated.summary, validated.keywords, digest, legacy), this.config.gitTimeoutMs);
    ...
    async replace(input) {
        ...
        return withLock(this.root, this.config, "replace", async () => {
            ...
            try {
                ...
                const hash = await commitWithIdentity(this.memDir, buildReplaceMessage(validated.title, validated.summary, validated.keywords, digest, full), this.config.gitTimeoutMs);
                await rm(journalPath(this.memDir), { force: true });
