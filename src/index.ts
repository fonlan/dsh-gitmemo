...
/** Stored-entry keywords (mem_write / mem_replace): 2-12, mirrors MAX_WRITE_KEYWORDS. */
const KEYWORDS_ARRAY = {
  type: "array",
  items: { type: "string" },
  description: "2-12 keywords (include 中英文 synonyms when useful)."
} as const;

/** Query keywords (mem_search): 1-15, mirrors MAX_SEARCH_KEYWORDS. */
const SEARCH_KEYWORDS_ARRAY = {
  type: "array",
  items: { type: "string" },
  description: "1-15 keywords (include 中英文 synonyms when useful)."
} as const;
...
function registerMemTools(ctx: { tools: { register(tool: unknown): unknown } }, config: ResolvedConfig): void {
  ...
  ctx.tools.register(defineTool({
      ...
      render: (
        ...
        if (value.links !== undefined && value.links.length > 0) {
          ...
          for (const link of value.links) {
            ...
            parts.push(
              "= " + link.resolved_hash.slice(0, 8) + " [" + (link.kind ?? "task") + "] " +
              (link.title ?? "") + " (" + state + ")"
            );
    ...
    isConcurrencySafe: () => true,
    async execute(args: { commit_hash: string; expand?: boolean }, exec: unknown) {
      const memo = await engineFor(exec, config);
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(
      ...
      return await memo.write({
        ...
        kind: (args as { kind?: "task" | "topic" }).kind
      });
    },
  ...
  ctx.tools.register(defineTool({
    name: "mem_replace",
    description:
      "Replace an active gitmemo memory entry with a new conclusion in ONE atomic commit (deletes the old entry file, adds the new one). Use it whenever a stored conclusion is superseded — the user corrects a stored outcome, OR this session's own work overrules it (implemented/refactored away) — never delete-then-write. commit_hash must be the ACTIVE hash returned by mem_search; stale hashes are rejected. Takes the same fields as mem_write; kind defaults to the replaced entry's kind (refreshing a topic page stays a topic page).",
    parameters: {
      commit_hash: {
        type: "string",
        required: true,
...
function registerPromptSection(ctx: { systemPrompt: { section(section: { name: string; order: number; text: string }): unknown } }): void {
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      "- BEFORE WORK — search: 仓库相关任务开始前提取 1–15 个中英文关键词调用 mem_search（含中英文同义词；搜索关键词上限高于写入上限，详见 KEYWORD SELECTION）；纯闲聊和通用问答无需搜索。",
      ...
      "- KEYWORD SELECTION — mem_write/mem_replace 的 keywords 应选择与任务相关但未出现在 title/summary 中的词（中英文同义词皆可，2–12 个）；mem_search 的查询词上限更高（1–15 个），多给近义词/中英对照可提升召回。title/summary 里已有的词本身就能被检索到，重复它们不会提高召回率。",
