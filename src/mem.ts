...
function git(dir: string, args: string[], timeoutMs: number): Promise<string> {
...
function isEntriesPath(memRoot: string, full: string): boolean {
 ...
 */
function entryFileFromPath(path: string, memDir: string, baseDir: string): string | undefined {
  if (path.length === 0) return undefined;
  const normalized = path.replaceAll("\\", "/");
  if (normalized.startsWith(".mem/entries/")) return normalized.slice(".mem/".length);
  if (normalized.startsWith("entries/")) return normalized;
...
function normalizeEntryFile(file: string, memDir: string, baseDir: string): string {
...
export class GitMemo {
  ...
  private readonly config: Required<GitMemoConfig>;
  ...
  private async currentBranch(dir: string): Promise<string> {
    try {
      ...
      if (branch.length === 0 || branch === "HEAD") return "main";
      return branch;
    } catch {
      ...
      try {
        ...
        if (sym.length > 0 && sym !== "HEAD") return sym;
      } catch {
  ...
  private async projectBranch(): Promise<string> {
    return this.currentBranch(this.root);
  }

  private async memBranch(): Promise<string> {
  ...
  private async syncBranch(): Promise<string> {
    if (!this.config.branchAlign) return this.memBranch();
   ...
   */
  async search(keywords: string, skip = 0, mode: "and" | "or" | "auto" = "auto"): Promise<SearchResult> {
    ...
    if (keywordsList.length === 0) throw new Error("gitmemo: search requires at least one non-empty keyword");
    if (!Number.isInteger(skip) || skip < 0) throw new Error("gitmemo: skip must be a non-negative integer");
    const normalizedMode = mode.toLowerCase() as "and" | "or" | "auto";
    if (!["and", "or", "auto"].includes(normalizedMode)) {
      throw new Error("gitmemo: mode must be one of: and, or, auto");
    }

    const run = async (searchMode: "and" | "or"): Promise<SearchHit[]> =>
      this.runSearchMode(keywordsList, searchMode, skip, limit);

    if (normalizedMode === "auto") {
      const andResults = await run("and");
      if (andResults.length >= 3) return { mode: "and", results: andResults };
      return { mode: "or", results: await run("or") };
    }
    return { mode: normalizedMode, results: await run(normalizedMode) };
  }

  private async runSearchMode(keywords: string[], mode: "and" | "or", skip: number, limit: number): Promise<SearchHit[]> {
  ...
  private acceptRecord(hash: string, subject: string, file: string, active: Set<string>): boolean {
    void hash;
    if (subject.startsWith("delete: remove")) return false;
    if (file.length === 0 || !active.has(file)) return false;
    return true;
  }

  /** Resolve the entry file touched by a commit (first file under entries/). */
  private async entryFileFromCommit(commitHash: string): Promise<string | undefined> {
    let output = "";
    try {
  ...
  async read(commitHash: string): Promise<{ file: string; content: string }> {
    if (!/^[0-9a-f]{7,}$/.test(commitHash)) throw new Error("gitmemo: read requires a commit hash");
    await ensureInit(this.memDir, this.config.gitTimeoutMs);
    const file = await this.entryFileFromCommit(commitHash);
    if (file === undefined) throw new Error("gitmemo: no entry file found in commit " + commitHash);
  ...
  async write(options: WriteOptions): Promise<WriteResult> {
    ...
    const resolvePayload = (p: string | undefined): string | undefined =>
      p === undefined ? undefined : (isAbsolute(p) ? p : resolve(this.root, p));
    ...
    if (!isSafeEntryPath(entryFile)) throw new Error("gitmemo: invalid file path: " + entryFile);
  ...
  async delete(commitHash: string): Promise<string> {
    if (!/^[0-9a-f]{7,}$/.test(commitHash)) throw new Error("gitmemo: delete requires a commit hash");
    await ensureInit(this.memDir, this.config.gitTimeoutMs);
    const file = await this.entryFileFromCommit(commitHash);
    if (file === undefined) throw new Error("gitmemo: no entry file found in commit " + commitHash);
...
export async function ensureInit(memDir: string, gitTimeoutMs = 60000): Promise<void> {
...
async function readFileSafe(path: string, errorPrefix: string): Promise<string> {
  try {
