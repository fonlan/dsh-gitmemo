...
import { migrateApply, migrateDryRun, type MigrationBaseline } from "./migrate.js";
...
function parseArgs(argv: string[]): CliArgs {
  const args: CliArgs = { projectRoot: "", action: "dry-run" };
  // optional subcommand token: `dsh-gitmemo migrate --project-root ...`
  if (argv[0] === "migrate") argv = argv.slice(1);
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--projectRoot": {
        ...
        if (value === undefined) throw new GitMemoError("gitmemo: --project-root requires a path");
      ...
      case "--sourceDir": {
        const value = argv[i + 1];
        if (value === undefined) throw new GitMemoError("gitmemo: --source requires a path");
        args.source = value;
        i += 1;
        break;
      }
      case "--out": {
        const value = argv[i + 1];
        if (value === undefined) throw new GitMemoError("gitmemo: --out requires a file path");
        args.out = value;
        i += 1;
        break;
      }
      case "--baseline": {
        const value = argv[i + 1];
        if (value === undefined) throw new GitMemoError("gitmemo: --baseline requires a file path");
        args.baseline = value;
        i += 1;
        break;
      }
...
function printDryRun(root: string, report: Awaited<ReturnType<typeof migrateDryRun>>): void {
...
async function main(): Promise<void> {
  ...
  if (args.baseline !== undefined) {
    try {
      baseline = JSON.parse(await readFile(args.baseline, "utf8")) as MigrationBaseline;
    } catch (error) {
